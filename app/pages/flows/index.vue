...
<template>
  <div class="space-y-6">
    ...
    <div v-else-if="flows.length === 0" class="flex flex-col items-center justify-center py-20 border-2 border-dashed rounded-xl bg-muted/20">
      ...
      >
        <div class="flex justify-between items-start mb-4">
          ...
          <div class="flex gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
            ...
            <Button variant="ghost" size="icon" class="h-8 w-8 text-destructive" @click.stop="deleteFlow(flow.id)">
        ...
        <div class="space-y-1">
          <div class="flex items-center gap-2">
            ...
            <span v-if="!flow.isActive" class="text-[10px] bg-muted px-1.5 py-0.5 rounded uppercase font-bold text-muted-foreground">Inativo</span>
          </div>
        ...
        <div v-if="flow.triggerEvents.length > 0" class="mt-4 flex flex-wrap gap-1">
          <span 
            v-for="event in flow.triggerEvents" 
            :key="event"
        ...
        <div class="mt-6 flex items-center justify-between border-t pt-4">
          ...
          <NuxtLink :to="`/flows/${flow.id}`">
            <Button variant="ghost" size="sm" class="h-8 group-hover:bg-primary group-hover:text-primary-foreground">
              Configurar
        ...
        <form @submit.prevent="createFlow" class="space-y-4">
          <div class="space-y-2">
            ...
            <input 
              ...
              placeholder="Ex: Sincronizar Novo Usuário"
              required
          ...
          <div class="space-y-2">
            ...
            <div class="flex flex-wrap gap-1 mt-2">
              <span 
                v-for="event in newFlow.triggerEvents" 
                :key="event"
                class="inline-flex items-center gap-1 text-[11px] bg-secondary text-secondary-foreground px-2 py-0.5 rounded-full"
              >
                {{ event }}
                <button type="button" @click="removeEvent(event)" class="hover:text-destructive">×</button>
              </span>
            </div>
            <p class="text-[10px] text-muted-foreground italic">Deixe vazio para disparo apenas manual via API.</p>
          </div>
